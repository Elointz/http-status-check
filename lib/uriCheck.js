...
// expectedText and notExpectedText is not empty then check that the expected/not expected text is/isn't in
// the response body received back from server.
function checkBody(site, body, accumulatedFails) {
  if(!site.expectedText && !site.notExpectedText) {
    return;
  }
  ...
  _.forEach(site.notExpectedText, function (textObj) {
    if (textObj.caseSensitive) {
      if (body.indexOf(textObj.text) > -1){
        accumulatedFails.push(
          'Not expected (case sensitive) text in response body was found: ' +
          textObj.text
        );
      }
    } else {
      ...
      if(lowerCaseBody.indexOf(textObj.text.toLowerCase()) > -1) {
        accumulatedFails.push(
          'Not expected (case insensitive) text in response body was found: ' +
          textObj.text.toLowerCase()
        );
      }
    }
